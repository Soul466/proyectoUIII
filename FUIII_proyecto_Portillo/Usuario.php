<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
    <title>39 Luis Carlos Portillo Jr</title>
    
	<link rel="stylesheet" href="estilos.css">
</head>
<body>
	<header>
		<div class="wrapp">
			<div class="logo">
				<a href="#"><img src="img/logo4.png" alt="FalconMasters"></a>
			</div>
			<nav>
				<ul>
				<li><a href="index.php">Inicio</a></li>
					<li><a href="profesores.php">Profesores</a></li>
					<li><a href="licencias.php">Licencias</a></li>
					<li><a href="autos.php">Autos</a></li>
					<li><a href="alumno.php">Alumnos</a></li>
				</ul>
			</nav>
		</div>
	</header>
	<section class="main">
		<div class="wrapp">
			<div class="mensaje">
				<h1>Clientes frecuentes:</h1>
			</div>

			<div class="articulo">
				<article>
					<p>Si estas registrado en nuestra plataforma porfavor inicia sesion aqui para poder ver tus datos(Solo clientes)</p>
					<br/>
					


<form action="welcome.php" method="post">
Usuario: <input type="text" name="name"><br>
Numero de cliente: <input type="text" name="name"><br>
E-Mail: <input type="text" name="name"><br>
Contraseña: <input type="text" name="email"><br>
Confirmar Contraseña: <input type="text" name="name"><br>
<input type="submit">
</form>



				</article>
			</div>

			<aside>
				<div class="widget">
					<h3>Secciones e informacion!</h3>
					<ul>
						<li><a href="#">Tabla 1 Empleado</a></li>
						<li><a href="#">Tabla 2 Usuario</a></li>
						<li><a href="#">Tabla 3 Agenda una cita </a></li>
						<li><a href="#">Tabla 4 informacion de contacto</a></li>
					</ul>
				</div>
			</aside>
		</div>
	</section>
	<footer>
		<div class="wrapp">
            <p> No lista 39 </p>
            <p> // SUBMÓDULO 2
                Desarrolla aplicaciones Web con conexión a bases de datos</p>
            <p>https://github.com/Soul466 </p>
		</div>
	</footer>
</body>
</html>